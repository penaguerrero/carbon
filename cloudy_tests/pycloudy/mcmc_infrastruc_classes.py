...
class PyCloudy_model:
    def __init__(self, model_name, dens, emis_tab, abunds, stb99_table, age, dir, verbosity, options, 
                 iterations, keep_files, n_proc=1):
        ...
        n_proc = number of processors, default is 1
        '''
...
class MCMC:
    ...
    def create_job_file(model_name):
        # this function creates the job file to be inputed to HTCondor
        job_file = model_name+'.job'
        jf = open(job_file, 'w+')
        print >> jf, 'Name =', model_name
    ...
    def run_chain(self):
        ...
        #p0 = [result["x"] + [np.random.uniform(9.5, 12.), np.random.uniform(7.5, 8.7), np.random.uniform(-1.6, 1.7), 
        #                     np.random.uniform(-1.7, 1.6), np.random.uniform(-1.0, 0.1), np.random.uniform(-2.3, -1.4)] for i in range(nwalkers)]

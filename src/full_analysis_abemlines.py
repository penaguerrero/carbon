...
def Halpha2Hbeta_dered(I_theo_HaHb, cHbeta, catalog_lines, normfluxes, norm_intensities, E_BV=None):
    ...
    for w, nF, nI in zip(catalog_lines, normfluxes, norm_intensities):    
        ...
        if E_BV != None:
            RC = pn.RedCorr(E_BV=E_BV, law= 'CCM 89', cHbeta=cHbeta)
        else:
...
def det_I_uncertainties(catalog_lines, normfluxes, all_err_cont_fit, verbose=False):
    ...
    faintest_line = min(normfluxes)
    if verbose == True:
        print 'very faintest faintest_line = ', faintest_line
    kk = copy.deepcopy(normfluxes)
    min_counts = 1
    end_loop = False
    while end_loop != True:
        for f in kk:
    ...
    absolute_Iuncert = []
    S2N = []
    for w, F_norm in zip(catalog_lines, normfluxes):
        if w <= 2000.0:
...
def corr_ColExcit(TO2gar, catalog_lines, element_lines, Idered, Hlines, verbose=False):
    '''
...
def find_Chi_of_CE(TO2gar, catalog_lines, element_lines, Idered, Hlines, theoCE, percent_Iuncert, verbose=False):
    # Correct for collisional excitation
    IcorrCE, obs_ratios, found_Hlines, norm_H6theo = corr_ColExcit(TO2gar, catalog_lines, element_lines, Idered, Hlines)
...
def redcor2(I_theo_HaHb, theoCE, Hlines, TO2gar, Idered, C_Hbeta, EWabsHbeta, catalog_lines, corr_undelyingAbs_EWs, 
    ...
    percent_Iuncert, _, _ = det_I_uncertainties(catalog_lines, normfluxes, all_err_cont_fit)
    Chi_sq, I_theo_H6Hb = find_Chi_of_CE(TO2gar, catalog_lines, element_lines, Idered, Hlines, theoCE, percent_Iuncert)
    ...
    for EWabsHbeta_iterations in range(0, number_iterations):
        ...
        Idered, _ = Halpha2Hbeta_dered(I_theo_HaHb, cHbeta, catalog_lines, normfluxes, norm_intensities)
        I_obs_H6Hb = catalog_lines[H6_idx] / catalog_lines[Hbeta_idx]
        Chi_sq, norm_H6theo = find_Chi_of_CE(TO2gar, catalog_lines, element_lines, Idered, Hlines, theoCE, percent_Iuncert)
        Chi_sq_models.append(Chi_sq)
    ...
    # Dered again and find the Chi_squared of that model
    norm_Idered, I_dered_norCorUndAbs = Halpha2Hbeta_dered(I_theo_HaHb, cHbeta, catalog_lines, normfluxes, norm_intensities)
    ...
    # Determine uncertainties    
    percent_Iuncert, absolute_Iuncert, S2N = det_I_uncertainties(catalog_lines, normfluxes, all_err_cont_fit)
...
def writeRedCorrFile(tfileRedCor, catalog_lines, element_lines, ion_lines, norm_Idered, absolute_Iuncert, verbose=False):
...
if emission_lines_only == True:
    # filter emission_lines_only and normalize to Hbeta
    lines = find_emission_lines(rounded_catalog_wavelength, element, ion, forbidden,
                                how_forbidden, observed_wavelength, flux, intensities, EW, continuum)
else:
    lines = normalize_lines(rounded_catalog_wavelength, element, ion, forbidden,
                            how_forbidden, observed_wavelength, flux, intensities, EW, continuum)
    
# expand the result
catalog_lines, wavs_lines, element_lines, ion_lines, forbidde_lines, how_forbidden_lines, normfluxes, calc_cont, norm_intensities, EW_lines = lines
# print in screen the line intensities and their error
lines_flux = []
for wa, fl in zip(rounded_catalog_wavelength, flux):
    if wa in catalog_lines:
        lines_flux.append(fl)
print 'Wavelength  flux      continuum     EW    normalized dereddend intensity'
for wa, fl, co, ew, I in zip(catalog_lines, lines_flux, calc_cont, EW_lines, norm_intensities):
...
norm_Idered, I_dered_norCorUndAbs = Halpha2Hbeta_dered(I_theo_HaHb, cHbeta, catalog_lines, normfluxes, norm_intensities, E_BV=ebv)
...
lines_pyneb_matches = writeRedCorrFile(tfile1stRedCor, catalog_lines, element_lines, ion_lines, norm_Idered, absolute_Iuncert, verbose=False)
...
if reddeningCorrection2 == True:
    ...
    lines_info, dereddening_info, uncertainties_info = redcor2(I_theo_HaHb, theoCE, Hlines, TO2gar, norm_Idered, C_Hbeta, EWabsHbeta, catalog_lines, corr_undelyingAbs_EWs, 
                rounded_catalog_wavelength, element, ion, forbidden, how_forbidden, observed_wavelength, flux, 
                intensities, EW, continuum, all_err_cont_fit)
    # separate the results
    catalog_lines, wavs_lines, element_lines, ion_lines, forbidden_lines, how_forbidden_lines, normfluxes, pos_calc_cont, norm_intensities, EW_lines = lines_info
    ...
    if print_2corr_file == True:
        lines_file = os.path.join(results4object_path, object_name+"_Case"+case+'_all_lines.txt')
        emfile = open(lines_file, 'w+')
        ...
        for w, el, i, f, h, ls, F_norm, I_norm, pu, au, eqw in zip(catalog_lines, element_lines, ion_lines, forbidden_lines, 
                                     how_forbidden_lines, flambdas, normfluxes, norm_Idered, percent_Iuncert, absolute_Iuncert, EW_lines):
            sn = F_norm/pu*100.
        ...
        lines_pyneb_matches = writeRedCorrFile(tfile_final_RedCor, catalog_lines, element_lines, ion_lines, norm_Idered, absolute_Iuncert, verbose=False)
